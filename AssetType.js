/*!
 * ------------------------------------------------------------------
 * The block immediately below is a small client-side router. It's
 * bundled directly into this file so there's only one script to
 * manage. Once loaded, it stays alive in the browser tab across a
 * soft-navigated trip to the Asset Details page and back - see the
 * comment above openSerialDetailsPage further down in this file for
 * how it's used, and search this file for "QafSoftNav" to find every
 ...
 * qaf-soft-nav.js
 * ----------------------------------------------------------------------
 * Lightweight, dependency-free "pjax" style router used to move between
 * the Asset Type (list) page and the Asset Details (serial) page without
 * a full browser reload.
 *
 * WHY THIS EXISTS
 * Both pages are independent server-rendered documents (their own
 * <head>/<body>, their own bundle/library/nav-dock script builds, their
 * own page-specific script). A plain <a href> / window.location.assign
 * therefore always throws the whole document away and starts a brand
 * new browsing context: white flash, every script re-parsed, every API
 * call the page makes on boot fired again from zero.
 *
 * This file does NOT change what either page renders or which APIs it
 * calls on boot - it only changes *how* we get from one page's markup
 * to the other's:
 *   1. fetch() the destination page's HTML (same document the browser
 *      would have navigated to),
 *   2. swap <body> (and any stylesheets it needs) in place,
 *   3. re-run that page's own <script> tags in their original order so
 *      it boots exactly like it would on a real load,
 *   4. reflect the change in the address bar via history.pushState and
 *      handle Back/Forward via popstate - so this is real client-side
 *      routing, not just a visual trick.
 *
 * Nothing here is page-specific. If the fetch fails, the response looks
 * wrong (non-2xx, redirected to a different page such as a login
 * screen, etc.), or anything throws, we fall back to a normal
 * window.location navigation - i.e. exactly today's behavior - so this
 * can never leave the user stuck.
 * ----------------------------------------------------------------------
 */
(function (global) {
  "use strict";

  if (global.QafSoftNav) return; // Already installed (script included twice) - no-op.

  var STYLE_WAIT_TIMEOUT_MS = 2000;
  var SOFT_NAV_STATE_MARKER = "__qafSoftNav";

  var nativeAddEventListener = global.EventTarget.prototype.addEventListener;
  var nativeRemoveEventListener = global.EventTarget.prototype.removeEventListener;

  var capturing = false;
  var capturedListeners = []; // {target, type, fn, options}
  var navigating = false;

  /* ------------------------------------------------------------------
     Listener bookkeeping
     ------------------------------------------------------------------
     Each page's own script attaches a handful of window/document level
     listeners (resize, popstate, visibilitychange, ...) the moment it
     runs. On a real navigation the whole JS context is thrown away, so
     those never need cleaning up. Here the tab/document stays alive, so
     if we swapped pages several times without cleanup those listeners
     would pile up (still pointing at the *previous* page's now-removed
     DOM). To keep this safe for repeated back-and-forth navigation, we
     temporarily patch addEventListener while a page's scripts are
     executing, record every window/document listener they add, and
     remove them again right before the *next* soft navigation swaps
     that content out.
  ------------------------------------------------------------------- */
  function beginCapture() {
    capturedListeners = [];
    capturing = true;
    global.EventTarget.prototype.addEventListener = function (type, fn, options) {
      if (capturing && (this === global || this === document)) {
        capturedListeners.push({ target: this, type: type, fn: fn, options: options });
      }
      return nativeAddEventListener.call(this, type, fn, options);
    };
  }

  function endCapture() {
    capturing = false;
    global.EventTarget.prototype.addEventListener = nativeAddEventListener;
  }

  function teardownCapturedListeners() {
    capturedListeners.forEach(function (entry) {
      try {
        nativeRemoveEventListener.call(entry.target, entry.type, entry.fn, entry.options);
      } catch (_error) {
        // Ignore - listener already gone or target rejects removal.
      }
    });
    capturedListeners = [];
  }
  ...
  function samePathname(urlA, urlB) {
    try {
      return new URL(urlA, location.href).pathname === new URL(urlB, location.href).pathname;
    } catch (_error) {
      return false;
    }
  }

  function fallbackToRealNavigation(url) {
    global.location.assign(url);
  }

  function loadStylesheetsFromHead(newHeadEl, baseUrl) {
    var existingHrefs = Array.prototype.map
      .call(document.head.querySelectorAll('link[rel="stylesheet"]'), function (el) {
        return el.href;
      });

    var newLinks = Array.prototype.slice.call(newHeadEl.querySelectorAll('link[rel="stylesheet"]'));
    var loads = [];

    newLinks.forEach(function (linkEl) {
      var href = absoluteUrl(linkEl.getAttribute("href"), baseUrl);
      if (!href || existingHrefs.indexOf(href) !== -1) return;

      var clone = document.createElement("link");
      clone.rel = "stylesheet";
      clone.href = href;
      if (linkEl.crossOrigin) clone.crossOrigin = linkEl.crossOrigin;
      ...
      var loadPromise = new Promise(function (resolve) {
        ...
        clone.addEventListener("load", finish);
        clone.addEventListener("error", finish);
        global.setTimeout(finish, STYLE_WAIT_TIMEOUT_MS);
      });
    ...
    // Inline <style> blocks (e.g. the preload-hiding rule some pages ship
    // in <head>) aren't covered by the href-based dedupe above - port any
    // whose exact text isn't already present.
    var existingInlineStyles = Array.prototype.map.call(
    ...
    Array.prototype.slice
      .call(newHeadEl.querySelectorAll("style"))
      .forEach(function (styleEl) {
        var text = styleEl.textContent || "";
        if (!text.trim() || existingInlineStyles.indexOf(text) !== -1) return;
        var clone = document.createElement("style");
        clone.textContent = text;
        document.head.appendChild(clone);
        existingInlineStyles.push(text);
      });

    return Promise.all(loads);
  }

  function extractBodyScripts(bodyEl) {
    var scripts = Array.prototype.slice.call(bodyEl.querySelectorAll("script"));
    var descriptors = scripts.map(function (scriptEl) {
      return {
        src: scriptEl.getAttribute("src") || "",
        inline: scriptEl.getAttribute("src") ? "" : scriptEl.textContent || ""
      };
    });
  ...
  function runBodyScriptsInOrder(descriptors, baseUrl) {
    return descriptors.reduce(function (chain, descriptor) {
      return chain.then(function () {
        return new Promise(function (resolve, reject) {
          var scriptEl = document.createElement("script");
          if (descriptor.src) {
            var absSrc = absoluteUrl(descriptor.src, baseUrl);
            if (!absSrc) {
              resolve();
              return;
            }
            ...
            scriptEl.addEventListener("error", function () {
              reject(new Error("Failed to load script: " + absSrc));
            });
            document.body.appendChild(scriptEl);
          } else {
            // Inline script (none expected in the pages this router
            // serves today, but handled for completeness).
            scriptEl.textContent = descriptor.inline;
            document.body.appendChild(scriptEl);
            resolve();
          }
        });
  ...
  /* ------------------------------------------------------------------
     Core swap
  ------------------------------------------------------------------- */
  function performSwap(html, responseUrl) {
    var parsed = new DOMParser().parseFromString(html, "text/html");
    if (!parsed || !parsed.body) throw new Error("Destination document could not be parsed.");

    return loadStylesheetsFromHead(parsed.head || parsed.documentElement, responseUrl).then(function () {
      // Teardown whatever the *previous* page's scripts attached to
      // window/document before we rip its DOM out from under it.
      teardownCapturedListeners();

      document.title = parsed.title || document.title;

      // Mirror the destination document's <html> classes (this is how
      // both pages hide themselves - e.g. "qaf-preload" - until their
      // own script is ready to reveal them; letting that class ride
      // along keeps that behavior identical to a real load).
      document.documentElement.setAttribute(
        "class",
        parsed.documentElement.getAttribute("class") || ""
      );

      var newBody = parsed.body;
      var scriptDescriptors = extractBodyScripts(newBody);

      document.body.setAttribute("id", newBody.getAttribute("id") || "");
      document.body.setAttribute("class", newBody.getAttribute("class") || "");
      document.body.innerHTML = newBody.innerHTML;

      beginCapture();
      return runBodyScriptsInOrder(scriptDescriptors, responseUrl).finally(endCapture);
    });
  }
  ...
  function fetchDocument(url) {
    return global
      ...
      .then(function (response) {
        if (!response.ok) throw new Error("Non-OK response (" + response.status + ") for " + url);
        // If the server redirected us somewhere other than the page we
        // asked for (auth gate, moved page, etc.) let a real navigation
        // handle it properly instead of soft-mounting the wrong page.
        if (response.redirected && !samePathname(response.url, url)) {
          throw new Error("Redirected away from the requested page.");
        }
  ...
  /* ------------------------------------------------------------------
     Public API
  ------------------------------------------------------------------- */
  // Tracks the path+search that is *currently rendered in the DOM* (as
  // opposed to what the address bar says, which is what drifts during a
  // Back/Forward popstate until we catch up to it below).
  var renderedPathAndSearch = location.pathname + location.search;

  function navigate(url, options) {
    var opts = options || {};
    var push = opts.push !== false;
    var absTarget = absoluteUrl(url, location.href);

    if (!absTarget) {
      fallbackToRealNavigation(url);
      return Promise.resolve(false);
    }

    var targetUrlObj;
    try {
      targetUrlObj = new URL(absTarget);
    } catch (_error) {
      fallbackToRealNavigation(url);
      return Promise.resolve(false);
    }

    if (targetUrlObj.origin !== location.origin) {
      fallbackToRealNavigation(url);
      return Promise.resolve(false);
    }

    var targetPathAndSearch = targetUrlObj.pathname + targetUrlObj.search;
    if (targetPathAndSearch === renderedPathAndSearch) {
      // Already showing this exact page/query - nothing to swap. Still
      // reflect it in history if this was an explicit forward navigation.
      if (push) {
        history.pushState(
          { path: targetPathAndSearch, marker: SOFT_NAV_STATE_MARKER },
          document.title,
          targetPathAndSearch + targetUrlObj.hash
        );
      }
    ...
    if (navigating) {
      // A navigation is already underway (e.g. a double click); don't
      // stack transitions on top of each other - just let the browser
      // do it the normal way instead of risking a torn UI state.
      fallbackToRealNavigation(url);
      return Promise.resolve(false);
    }
    ...
    return fetchDocument(absTarget)
      .then(function (result) {
        return performSwap(result.html, result.url).then(function () {
          renderedPathAndSearch = targetPathAndSearch;
          if (push) {
            history.pushState(
              { path: targetPathAndSearch, marker: SOFT_NAV_STATE_MARKER },
              document.title,
  ...
  /* ------------------------------------------------------------------
     Back / Forward
     ------------------------------------------------------------------
     history.back() (used by the Asset Details page's Back link) pops
     the pushState entry we created above and fires popstate here in the
     *same* document - no network navigation happens on its own. We
     re-run the same fetch+swap for whatever URL the browser just moved
     to, so Back feels identical to Forward: instant, same-document,
     only that page's own APIs fire.
  ------------------------------------------------------------------- */
  global.addEventListener("popstate", function () {
    if (navigating) return; // let the in-flight transition finish first
...
(() => {
  ...
  function openSerialDetailsPage(recordKey) {
    ...
    /*
     * Soft-navigate via the router bundled at the top of this file: this
     * fetches AssetDetails' markup and swaps it in place instead of doing
     * a full browser reload, so the page transition is instant and only
     * the Asset Details page's own APIs fire on the way in. That router
     * also owns Back/Forward (via popstate) for the whole round trip, so
     * nothing else needs to change on the Asset Details side. If the
     * router isn't present for any reason, this is exactly the original
     * behavior - a normal same-tab navigation.
     */
    if (window.QafSoftNav && typeof window.QafSoftNav.navigate === "function") {
      window.QafSoftNav.navigate(destination).catch(() => {
  ...
  /** Page size for `api/rnsp` PageSize: the route's `count` query param when present (e.g. `?count=12`), else the default PAGE_SIZE. */
